...
const ProductsPage = () => {
  ...
  const { search: searchQuery, category: categoryParam, brand: brandParam, ram: ramParam } = router.query;
  ...
  useEffect(() => {
    ...
    if (brandParam && typeof brandParam === 'string' && !selectedBrands.includes(brandParam)) {
      setSelectedBrands(prev => [...prev, brandParam]);
    }
    
    if (ramParam && typeof ramParam === 'string' && !selectedRam.includes(ramParam)) {
  ...
  }, [router.isReady, router.query, selectedBrands, selectedRam, priceRange, sortOption]);
